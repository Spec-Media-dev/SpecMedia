...
"""
    ...
    if client_reviews:
        ...
        for rev in reviews_pool:
            ...
            if current_lang == 'ar':
                ...
                ig_cards.append(
                    f'<article data-rv="1" class="ig-card" style="flex:none;width:clamp(310px,26vw,380px);background:#141312;border:1px solid rgba(255,255,255,0.08);border-radius:18px;overflow:hidden;box-shadow:0 18px 46px rgba(0,0,0,0.6);display:flex;flex-direction:column;direction:rtl;text-align:right;font-family:\'Cairo\',sans-serif;transition:transform .35s cubic-bezier(.16,1,.3,1),box-shadow .35s ease,border-color .35s ease;will-change:transform">'
                    ...
                    f'<span class="ig-handle" style="font-family:system-ui,-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif;font-size:13px;font-weight:700;color:#ffffff;direction:ltr;unicode-bidi:isolate">{handle}</span>'
                    ...
                    f'</div><div style="font-family:\'Cairo\',sans-serif;font-size:10px;color:#a89f91;letter-spacing:0;font-weight:500;margin-top:2px">{role}</div></div></div>'
                    f'<div style="color:#a89f91;cursor:pointer;padding:4px;display:flex;gap:3px"><span style="width:3px;height:3px;border-radius:50%;background:currentColor"></span><span style="width:3px;height:3px;border-radius:50%;background:currentColor"></span><span style="width:3px;height:3px;border-radius:50%;background:currentColor"></span></div></div>'
                    ...
                    f'<button type="button" class="ig-btn-like" onclick="toggleIGLike(this)" style="background:none;border:none;padding:0;cursor:pointer;color:#ffffff;display:flex;align-items:center;transition:transform .2s ease" aria-label="Like">'
                    ...
                    f'<button type="button" style="background:none;border:none;padding:0;cursor:pointer;color:#ffffff;display:flex;align-items:center" aria-label="Comment">'
                    ...
                    f'<button type="button" style="background:none;border:none;padding:0;cursor:pointer;color:#ffffff;display:flex;align-items:center" aria-label="Share">'
                    ...
                    f'<button type="button" class="ig-btn-save" onclick="toggleIGSave(this)" style="background:none;border:none;padding:0;cursor:pointer;color:#ffffff;display:flex;align-items:center" aria-label="Save">'
                    ...
                    f'<div style="padding:0 14px 4px;font-family:\'Cairo\',sans-serif;font-size:12px;color:#ffffff;font-weight:600;letter-spacing:0;text-align:right">'
                    ...
                    f'<div style="padding:2px 14px 8px;font-family:\'Cairo\',sans-serif;font-size:13px;line-height:1.65;color:#ffffff;text-align:right">'
                    ...
                    f'<span style="color:#ded3c3;font-weight:400">{quote}</span></div>'
                    ...
                    f'<div style="font-family:\'Cairo\',sans-serif;font-size:11px;color:#e26b72;font-weight:600;letter-spacing:0">{tags}</div></div>'
                    f'<div style="padding:0 14px 14px;display:flex;flex-direction:column;gap:4px;text-align:right">'
                    f'<span style="font-family:\'Cairo\',sans-serif;font-size:11px;color:#8c8273;cursor:pointer;font-weight:500">عرض جميع التعليقات</span>'
                    f'<span style="font-family:\'Cairo\',sans-serif;font-size:10px;letter-spacing:0;color:#70685c;font-weight:600">{time_ago}</span></div>'
                    f'</article>'
                )
            else:
                ...
                ig_cards.append(
                    f'<article data-rv="1" class="ig-card" style="flex:none;width:clamp(310px,26vw,380px);background:#141312;border:1px solid rgba(255,255,255,0.08);border-radius:18px;overflow:hidden;box-shadow:0 18px 46px rgba(0,0,0,0.6);display:flex;flex-direction:column;transition:transform .35s cubic-bezier(.16,1,.3,1),box-shadow .35s ease,border-color .35s ease;will-change:transform">'
                    f'<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,0.06)">'
                    f'<div style="display:flex;align-items:center;gap:10px">'
                    f'<div style="width:38px;height:38px;border-radius:50%;padding:2px;background:linear-gradient(45deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%);display:grid;place-items:center;flex-shrink:0">'
                    f'<div style="width:100%;height:100%;border-radius:50%;overflow:hidden;border:2px solid #1a1816;background:#2a2622">'
                    ...
                    f'<span style="font-family:system-ui,-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif;font-size:13px;font-weight:700;color:#ffffff">{handle}</span>'
                    f'<svg width="13" height="13" viewBox="0 0 24 24" fill="#3897f0" style="flex-shrink:0"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15l-5-5 1.41-1.41L11 14.17l7.59-7.59L20 8l-9 9z"/></svg>'
                    f'</div><div style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;color:#a89f91;letter-spacing:.02em">{role}</div></div></div>'
                    f'<div style="color:#a89f91;cursor:pointer;padding:4px;display:flex;gap:3px"><span style="width:3px;height:3px;border-radius:50%;background:currentColor"></span><span style="width:3px;height:3px;border-radius:50%;background:currentColor"></span><span style="width:3px;height:3px;border-radius:50%;background:currentColor"></span></div></div>'
                    f'<div style="position:relative;aspect-ratio:4/3;background:#151413;overflow:hidden;cursor:pointer" ondblclick="handleIGCardDblClick(this)">'
                    f'<img data-media="1" src="{img}" alt="{handle} Showcase" draggable="false" style="width:100%;height:100%;object-fit:cover;display:block;transition:transform .7s cubic-bezier(.16,1,.3,1)">'
                    f'<div class="ig-heart-pulse" style="position:absolute;inset:0;display:grid;place-items:center;pointer-events:none;opacity:0;transform:scale(0.3);transition:all .35s cubic-bezier(.175,.885,.32,1.275)">'
                    ...
                    f'<button type="button" class="ig-btn-save" onclick="toggleIGSave(this)" style="background:none;border:none;padding:0;cursor:pointer;color:#ffffff;display:flex;align-items:center" aria-label="Save">'
                    f'<svg class="save-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg></button></div>'
                    f'<div style="padding:0 14px 4px;font-family:system-ui,-apple-system,BlinkMacSystemFont,sans-serif;font-size:12px;color:#ffffff;font-weight:600">'
                    f'Liked by <span style="font-weight:700">specmedia</span> and <span class="like-number" data-count="{likes}">{likes:,}</span> others</div>'
                    f'<div style="padding:2px 14px 8px;font-family:system-ui,-apple-system,BlinkMacSystemFont,sans-serif;font-size:13px;line-height:1.55;color:#ffffff">'
                    f'<span style="font-weight:700;margin-right:6px">{handle}</span><span style="color:#ded3c3;font-weight:400">{quote}</span></div>'
                    ...
                    f'<div style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;color:#e26b72">{tags}</div></div>'
                    f'<div style="padding:0 14px 14px;display:flex;flex-direction:column;gap:4px">'
                    f'<span style="font-family:system-ui,-apple-system,sans-serif;font-size:11px;color:#8c8273;cursor:pointer">View comments</span>'
                    f'<span style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:#70685c">{time_ago}</span></div>'
                    f'</article>'
                )
        if ig_cards:
    ...
    if capability_photos:
        ...
        idle_html = '<div data-pvidle="1" style="position:absolute;inset:0;display:grid;place-items:center;background:#141312;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#8c8273;transition:opacity .45s ease">SELECT A DISCIPLINE</div>'
        all_panels = '\n          '.join(panels_html) + '\n          ' + idle_html
    ...
    if projects:
        ...
        for index, project in enumerate(projects):
            ...
            tag_markup = ''.join(
                f'<span style="padding:8px 14px;background:color-mix(in oklab, var(--color-text) 78%, transparent);backdrop-filter:blur(6px);color:#ffffff;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;letter-spacing:.18em;text-transform:uppercase;font-size:10px">{escape(tag)}</span>'
                for tag in tags
            )
            ...
            cards.append(
                ...
                f'<h3 style="margin:0;font-family:var(--font-heading);font-size:clamp(22px,2.4vw,34px);line-height:1.05;color:#ffffff">{escape(project.title)}</h3></div>'
                f'<p style="margin:0;padding-left:17px;max-width:44ch;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;letter-spacing:.18em;text-transform:uppercase;font-size:11px;line-height:1.9;color:#a89f91">{escape(project.summary)}</p></article>'
            )
